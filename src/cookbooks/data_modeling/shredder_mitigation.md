# Shredder mitigation during backfills
...
This documentation provides step-by-step guidance with example and common scenarios, to help you achieve consistent results.
...
Some examples of aggregates where this process is applicable
...
- Propagate upstream changes in business definitions to an existing dimension or metric in the aggregate.

## How this process transforms your workflow

- With this process, you can now safely backfill aggregate tables, which previously carried high risks, potentially affecting business KPIs, metrics and forecasting.
...
- Subtotals adjusted for modified or newly added columns, with NULL values increasing by the amount corresponding to the shredded client_ids.
...
- `segment` where the business logic for all browsers changed in 2024-H1 to integrate the dau definition the segmentation of clients.
- `os_version` where the logic requires an integration of `build_version` for Windows operating systems.
...
1. Make the necessary updates to the new version of the query and schema:
   ...
   - Update existing columns and rename them in the file (e.g. rename `os_version` to `os_version_build`) to ensure they are also recalculated. If you need to avoid changes in Looker, set the expected column name in the view.
1. Use a [managed backfill](https://mozilla.github.io/bigquery-etl/cookbooks/creating_a_derived_dataset/#backfilling-a-table) for the new query version, using the `--shredder_mitigation` parameter.
...
2. The process halts and returns a detailed failure message that you can resolve by providing the missing information or correcting the issue:
   ...
   - The metadata of the aggregate to backfill does not contain the `shredder_mitigation` label.
   - The previous or new version of the query contains an invalid GROUP BY for this process, such as GROUP BY 1, 2 or GROUP BY ALL.
...
This section contains examples both for regular runs and first-time runs of a backfill with shredder mitigation.

The sample requirement is to modify `firefox_desktop_derived.active_users_aggregates_vX` to:

- Update the definition of existing column `os_version`.
- Remove column `attributed`.
- Backfill data.

After running the backfill, you should expect the distribution of `os_version` to change, with an increase in NULL values. Additionally, column `attributed` will no longer be present, and the totals for each column will remain unchanged.
...
That's all! The aggregate table is set and ready for simplified subsequent backfills with shredder mitigation.

### Example 2. First-Time run

This example is for a first-run of the process, which requires setting up the table appropriately and propagating upstream changes. Customize this comprehensive example to your requirements.
...
In preparation for the process itself, the first step is to get the table ready for the backfill by making these initial changes:

- Replace `GROUP BY ALL` with the explicit list of columns in both the previous and new version of the query.
- Replace the definition of `city` to `IFNULL(city, '??') AS city` to apply consistent logic in all versions and existing data.
- Add descriptions to all columns in the schema.
- Include label `shredder-mitigation: true` to the metadata of the table.
- Raise and merge a Pull Request to apply these changes.

##### Changes related to the requirement

- Bump the version of the table and make changes to the new version files.
- Update the definition of `os_version` in the query and rename it to `os_version_build` both in query and schema to ensure that it is recalculated. The schema description remains valid.
- Remove column `attributed` from the query and schema.

##### Changes to update columns with upstream changes yet to be propagated:

- Rename dimensions with known upstream changes that haven't propagated to our aggregate. To achieve this, `first_seen_year` is renamed to `first_seen_year_new` and `segment` is renamed to `segment_dau`.
- Raise a PR to apply changes.
- Follow the [managed backfill](https://mozilla.github.io/bigquery-etl/cookbooks/creating_a_derived_dataset/#backfilling-a-table) process with `--shredder_mitigation` parameter.
