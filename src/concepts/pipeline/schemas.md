...
Data that fails validation is sent to the `payload_bytes_error` table. Each row contains
an information about the error that caused it, as well as the name of the job associated with it.
...
Snake casing is employed across all schemas, which ensures that nested structures can always be
accessed using the dot operator. Some data must be transformed before insertion, such as map-types
(a.k.a. dictionaries in Python), due to limitations in BigQuery data representation. Other data may
